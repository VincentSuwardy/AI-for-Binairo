...
class Genetic:
    '''
        Set Genetic Algorithm Hyperparameter
    '''
    def set_ga_params(population_size, max_generations, mutation_rate, crossover_rate, elite_size, adaptive):
    ...
    # count total empty cells in board
    ...
    '''
    def get_ga_params(empty, difficulty=""):
        # size = rows * cols
        size = empty
        ...
        pop_size = min(max(pop_size, POPULATION_SIZE), 1000)
        max_gen = min(max(max_gen, MAX_GENERATIONS), 2000)

        elite_size = max(1, int(pop_size * 0.1))
        return pop_size, max_gen, elite_size

    '''
        initialization population
        - fill the rest EMPTY cell(s) with random WHITE or BLACK
    '''
    def init_population(self, base_board, fixed_mask, pop_size):
        population = []
    ...
    def cached_fitness(self, board, w1, w2, w3, cache):
        key = tuple(tuple(row) for row in board)

        if key not in cache:
            cache[key] = self.fitness(board, w1, w2, w3)
    ...
    '''
    def tune_fitness_weights(self, sample_board, fixed_mask, iterations=30):
        # initial weights
        current = [20, 10, 1]
        ...
        def evaluate(weights):
            w1, w2, w3 = weights
            pop = self.init_population(sample_board, fixed_mask, 30)

            scores = [self.fitness(b, w1, w2, w3) for b in pop]
    ...
    '''
    def selection(self, population, num_pairs):
        pairs = []

        # count every indiv fitness
        fitness_values = [self.fitness(idx) for idx in population]
    ...
    '''
    def crossover(self, parent1, parent2, fixed_mask):
        if (random.random() > 0.5):
            return self.row_based_crossover(parent1, parent2, fixed_mask)
        else:
            return self.col_based_crossover(parent1, parent2, fixed_mask)
    ...
    '''
    def mutate(self, board, fixed_mask):
        conflicts = self.get_conflict_cells(board)
    ...
    '''
    def run_genetic(self, base_board, fixed_mask, difficulty):
        rows = len(base_board)
        cols = len(base_board[0])

        if ADAPTIVE[0]:
            pop_size, max_gen, elite_size = self.get_ga_params(
                self.count_cells(base_board),
                difficulty
            )
        else:
        ...
        population = self.init_population(base_board, fixed_mask, pop_size)

        best = None
        best_score = float('-inf')
        fitness_cache = {}

        # tuning weights jika ADAPTIVE 
        w1, w2, w3 = self.tune_fitness_weights(base_board, fixed_mask) if ADAPTIVE[1] else (20, 10, 1)
        # w1, w2, w3 = self.tune_fitness_weights(base_board, fixed_mask)
        ...
        for gen in range(max_gen):
            ...
            # elitism
            population.sort(
                key=lambda x: self.cached_fitness(x, w1, w2, w3, fitness_cache)
            )

            best_current = population[0]
            best_current_score = self.cached_fitness(best_current, w1, w2, w3, fitness_cache)
            ...
            if best_score == 0:
                return best, best_score, (w1, w2, w3)
            ...
            pairs = self.selection(population, pop_size-elite_size)

            for parent1, parent2 in pairs:
                if random.uniform(0.0, 1.0) < CROSSOVER_RATE:
                    child = self.crossover(parent1, parent2, fixed_mask)
                else:
                    child = copy.deepcopy(
                        parent1 if self.fitness(parent1) < self.fitness(parent2) else parent2
                    )

                if random.uniform(0.0, 1.0) <= MUTATION_RATE:
                    child_before = copy.deepcopy(child)
                    old_fit = self.fitness(child_before)

                    self.mutate(child, fixed_mask)

                    new_fit = self.fitness(child)

                    # kalau fitness nya lebih jelek, buang hasil mutate nya
                    if new_fit > old_fit:
                        child = child_before
